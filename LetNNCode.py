...
def network(trainingDat, trainingLab, testDat, testLab):
    ...
    network = Sequential([ #creates a sequential CNN with various conv, pooling, dropout, and dense layers
        ...
        MaxPooling2D(2,2),
        Flatten(),
        Dense(676, activation = tf.nn.relu),
    ...
    network.fit(trainingDat, trainingLab, epochs = 20, callbacks = [CallbackTraining])
    network.evaluate(testDat, testLab)
    return network